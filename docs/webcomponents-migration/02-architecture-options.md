...
| Bundle size            | **M** 11.06 kB / 5 atoms, +6.88 kB `lit` runtime once                                          | **M** Same, + `@lit/react` glue                        | **M** Same as A                                              | **R** No `lit` runtime saving                               | **—** Would isolate the 6.88 kB question           | **R** Both costs, no saving           | **M** Core is small; adapters pay their own                        | **R** Zero JS                            |
...
2. **Bundle size improves by an order of magnitude per component.** 7.4×–16.2× smaller per atom;
   17.94 kB for 5 atoms including the `lit` runtime against 105.72 kB (**measured**,
   `10-performance-report.md`, which supersedes §3's pre-646b figures).
