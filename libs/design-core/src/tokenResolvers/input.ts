 ...
 * resolve `gd-design-library/tokens` + `resolveThemeTree` the way `gd-input.ts` (Lit) does —
 * see this package's README "Status"/"Theme parameter" sections for the accepted duplication
 * trade-off. Any edit to `libs/ui/src/tokens/input.ts` will NOT automatically propagate here;
 ...
 * `react-native`/`select.ts` still consume it. The debounce/interaction-tracking behavior
 * that gives Input its real portability value lives in `stores/createInputStore.ts`, unaffected
 * by this recreation — this resolver only covers the static style values every adapter needs
